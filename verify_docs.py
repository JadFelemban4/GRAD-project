...
def dwell_column(S):
    """Seconds of unbroken running above ENR_LOAD (180 kPa), per drive.

    Was 200 kPa until 10 September. The threshold moved with the manifold
    pressure definition, not with the physics -- see engine_env.ENR_LOAD.
    """
    out = []
    for _, d in S.groupby("source"):
        d = d.sort_values("t").reset_index(drop=True)
        acc, run = 0.0, np.zeros(len(d))
        for i, v in enumerate((d.map_kpa > 180).fillna(False)):
...
RETIRED = [
    ...
    (r"\b118 seconds?\b", "118 s above 230 kPa (7 drives)", "178 s above 207 kPa"),
    (r"\b198 seconds\b", "198 s, before the charge-temp correction", "178 s"),
    (r"\+0\.02\s*[—-]\s*none at all", "corr(lambda, MAP) = +0.02", "+0.23"),
    (r"corr\(.{0,12}MAP.{0,12}\)\s*[-−]0\.05", "corr(lambda, MAP) = -0.05, pre-correction", "+0.23"),
    (r"31\.4\s*[-–]\s*81\.6|31\s*[-–]\s*82 kPa", "the range with the sensor as charge temp", "30-74 kPa"),
    ...
    (r"eight drives, 168\.1 minutes", "dataset size before pull01", "nine drives, 175.5 minutes"),
...
def main():
    ...
    chk("total minutes", round(float(M.duration_min.sum()), 1), 175.5, 0.15, " min")
    chk("drives in the manifest", len(M), 9)
    ...
    chk("drives showing that exact ceiling", hits, 6)
    ...
    for lo, hi, claim in ((1000, 3500, 422), (3500, 4500, 168), (4500, 7000, 465)):
        chk(f"samples, {lo}-{hi} rpm above 180 kPa",
            int(((h.rpm > lo) & (h.rpm <= hi)).sum()), claim)
    hh = S2[(S2.map_kpa > 207) & S2.lam.between(0.5, 1.3)]
    chk("seconds above 207 kPa", round(len(hh) / 4.6), 178, 1, " s")
    v = h[["lam", "rpm", "air_gps", "dwell", "map_kpa"]].dropna()
    # The docstring quotes the >180 kPa subset, matching ENR_LOAD. Keep it that way.
    for col, claim in (("rpm", -0.56), ("air_gps", -0.49),
                       ("dwell", -0.47), ("map_kpa", +0.23)):
    ...
    # k is a ratio of air densities, so it is DERIVED, not fitted. T_in here is
    # the MODELLED charge temperature -- the same one compare_log.py uses, from
    # plant.charge_temperature -- not the pre-throttle sensor, which is a
    # compressor outlet. Using the sensor gave 0.783; the corrected value is
    # 0.829, and the fitted constant moved with it from 0.784 to 0.837.
    ...
    chk("derived k = 269.6 / T_charge, mean over the 22 points",
    ...
    # fails loudly instead of the error hiding inside a 1.4 % that cannot see it.
    ...
    chk("boosted model vs the car's own boost channel",
        round(float(gap), 1), 3.0, 0.4, " %  (was +22.6 with the sensor)")
