...
def main():
    ...
    # DERIVED, never a literal. This line printed "12 % at 130 km/h" as a
    # hardcoded string while taking make_grade_climb's DEFAULT speed, so on a
    # branch whose default is 110 it reported a scenario it was not running.
    # It is read off the cycle now and cannot disagree with what was scored.
    _c = make_grade_climb(duration=DURATION, dt=DT)
    _v = float(np.max(_c["v_mps"]) * 3.6)
    _g = float(np.max(_c["grade"]) * 100.0)
    _t = float(_c["t_amb"] - 273.15)
    print(f"scenario: {_g:.0f} % at {_v:.0f} km/h, {_t:.0f} C, "
