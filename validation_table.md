...
(The ninth, `pull01`, is a 7-channel physics drive with no coolant channel; it
contributes no samples and no operating points by design — see CLAUDE.md.)
`build_dataset.py` finds **22 distinct operating points**, 30–74 kPa manifold
pressure. <!-- RETIRED-OK --> (31–82 kPa before the 10 September charge-temperature
correction; the points are the same, the pressure scale changed.)
...
MODELLED charge temperature, not a logged channel. All 22 points sit at
30–74 kPa. Note that T_in cancels out of the derived form, so what stops this
being extended into boost is the MAF ceiling and the logger's sampling, not the
temperature.
